...
class DannFixbiModule(LightningModule):
    def __init__(self, params):
        super().__init__()
        # models
        # self.source_encoder = ResNet50()
        # self.target_encoder = ResNet50()
        self.source_encoder = Extractor()
        self.target_encoder = Extractor()
        self.source_classifier = Classifier(params.num_classes)
        self.target_classifier = Classifier(params.num_classes)
    ...
    def training_step(self, batch, batch_idx):
        ...
        x_sd_repr = self.source_encoder(x_target)
        x_td_repr = self.target_encoder(x_target)
        x_sd = self.source_classifier(x_sd_repr)
        x_td = self.target_classifier(x_td_repr)

        # for souce domain
        pseudo_sd, top_prob_sd, threshold_sd = self.get_target_preds(x_sd)
        fixmix_sd_loss, fixmix_sd_domain_loss = self.get_fixmix_loss(self.source_encoder, self.source_classifier,
                                                                     self.source_discriminator, alpha, x_source, x_target, y_source, pseudo_sd, self.lam_sd)

        self.log_dict({"fixmix_loss(SDM)": fixmix_sd_loss,
                      "domain_loss(SDM)": fixmix_sd_domain_loss}, on_epoch=True, on_step=False)

        # for target domain
        pseudo_td, top_prob_td, threshold_td = self.get_target_preds(x_td)
        fixmix_td_loss, fixmix_td_domain_loss = self.get_fixmix_loss(self.target_encoder, self.target_classifier,
                                                                     self.target_discriminator, alpha, x_source, x_target, y_source, pseudo_td, self.lam_td)
        self.log_dict({"fixmix_loss(TDM)": fixmix_td_loss,
                      "domain_loss(TDM)": fixmix_td_domain_loss}, on_epoch=True, on_step=False)

        # loss contains cross entropy loss for source and target domains
        total_loss = fixmix_sd_loss + fixmix_td_loss + \
            fixmix_sd_domain_loss + fixmix_td_domain_loss
    ...
    def get_fixmix_loss(self, encoder, classif, disrim, alpha, src_imgs, tgt_imgs, src_labels, tgt_pseudo, ratio):
        mixed_x = ratio * src_imgs + (1 - ratio) * tgt_imgs
        mixed_x_enc = encoder(mixed_x)
        mixed_x = classif(mixed_x_enc)
        ...
        domain_source_labels = torch.zeros(
            src_labels.shape[0]).type(torch.LongTensor).to(mixed_x.get_device()).detach()
        domain_target_labels = torch.ones(
            tgt_pseudo.shape[0]).type(torch.LongTensor).to(mixed_x.get_device()).detach()
        domain_logits = disrim(mixed_x_enc, alpha)
        domain_loss = ratio * self.CrossEntropy(domain_logits, domain_source_labels) + (
            1 - ratio) * self.CrossEntropy(domain_logits, domain_target_labels)
    ...
    def evaluate(self, batch, stage=None):
        ...
        #   print("something")
        logits_sdm = self.source_classifier(self.source_encoder(x))
        logits_tdm = self.target_classifier(self.target_encoder(x))
